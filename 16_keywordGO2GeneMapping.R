...
load(file='results/fit.stan.nb_13Nov.rds')
...
#dfGO = AnnotationDbi::select(org.Mm.eg.db, keys = 'Itgav', columns = c('GO'), keytype = 'SYMBOL')
...
dim(dfGO)
## keyword search
i = grep('interleukin', dfGO$DEFINITION, ignore.case = T)
length(i)
temp = dfGO[i,]
...
head(d.bk)
d.bk$coef = colMeans(mCoef[,d.bk$cols])
temp = gsub('(WT:|miR-142 KO:).+', '\\1', as.character(d.bk$fBatch))
temp = gsub(':', '', temp)
d.bk$differentiated = factor(temp, levels=c('WT', 'miR-142 KO'))

temp = gsub('WT:|miR-142 KO:', '', as.character(d.bk$fBatch))
...
xyplot(coef ~ differentiated | SYMBOL, groups=groups, data=d.bk, type=c('l', 'p'), scales=list(relation='free', x=list(cex=0.7), y=list(cex=0.7)), 
       ylab='Model Estimated log Deflections from Intercept', main=list(label='profile of selected genes', cex=0.8),
       auto.key=list(columns=3), xlab='Genotype')
