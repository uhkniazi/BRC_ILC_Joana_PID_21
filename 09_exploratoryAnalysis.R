...
fBatch = factor((gsub('\\d+.+(HOM|WT_KLS_\\d).+(KLS_\\d)', '\\1_\\2', dfSample$group3)))
...
levels(fBatch)
fBatch = fBatch:factor(dfSample$group2)
...
xtabs( ~ group3 + group1, data=dfSample.2)
...
fBatch = factor((gsub('\\d+.+(HOM|WT_KLS_\\d).+(KLS_\\d)', '\\1_\\2', dfSample.2$group3)))
cvDesc = dfSample.2$description
lDesc = strsplit(cvDesc, ';')
fBatch = factor(sapply(lDesc, function(x) return(x[4])))
levels(fBatch)
fBatch = fBatch:factor(dfSample.2$group2)
xtabs(~ group3+group2, data=dfSample.2)
