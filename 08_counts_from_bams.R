...
where (Sample.idData = 42) AND (File.idSample = Sample.id AND File.type like "%duplicates removed%")')
...
setwd(gcswd)
n = make.names(paste('lCounts rd did 42 joana rds'))
...
#                 comment='list of Count matrix from joana mouse ILC project with quality 10 and duplicates removed')
